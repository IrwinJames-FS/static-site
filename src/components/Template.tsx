...
<body className={`${inter.className} h-dvh bg-slate-100 flex flex-col justify-start`}>
  <header className="h-64 lg:h-16 bg-slate-200 flex flex-col lg:flex-row justify-start lg:justify-between items-center px-2 shadow lg:fixed w-dvw z-50">
	...
	<nav className="flex flex-col lg:flex-row items-stretch">
		{navLinks.map(({title, href}, i)=><Link key={`nv-${i}`} href={href} className={`text-center${i < navLinks.length-1 ? ' lg:border-r-2 border-slate-300':''}`}>{title}</Link>)}
	</nav>
  </header>
